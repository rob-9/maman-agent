...
echo ""
echo "WARNING: a rebuild changes the app's CDHash, and macOS may re-key keychain"
echo "access to it — the next launch can require re-approving access to the"
echo "'com.maman.desktop.keys' item, and the \"Always Allow\" dialog sometimes"
echo "NEVER renders for an accessory-style app. Maman surfaces this honestly"
echo "(red dot, 'needs keychain access') instead of hanging; if you see it,"
echo "relaunch Maman from Finder and click \"Always Allow\" when prompted."
echo "Never delete the keychain item to 'fix' it — that orphans the encrypted store."
