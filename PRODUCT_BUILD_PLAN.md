...
| Corrections: the agent learns from what the person changed          | ✅ one record for every pair of what the agent proposed and what the person did, whatever the output: a draft against the message sent (shorter, greeting, sign-off, opener, as signals, never the text), a proposed field against a decline or a hand edit, a routine's steps against the steps taken (migration 0020); one inference over them: three of the same change become a proposal in "Your agent thinks" ("Sign off with 'Best, Alex'", "Keep drafts short, about 60 words", "Don't propose changes to the close date"), kept ones reach the writer and the reader as the person's instruction, a kept skip-field rule is enforced where proposals are made; what the person sent after editing leads the voice examples; 3 + 5 unit, 2 + 3 integration, 3 drilled                                                                                                                                                                                                                                                       |
| Phase 3, step 5 begins: sending mail, behind a per-person promotion | ✅ a `gmail.send` capability and action kind (high risk, not reversible, never retried); a send is proposed as the exact draft (the text as a hash, shown in full on the page), approved bound to that hash, sent once by draft id, read back from Gmail before it counts, and recorded with no undo; refused when the draft was edited since, the thread moved, or the item is no longer pending; a failed send is never proposed again by anyone; "Always" per situation (reply owed, gone quiet, after a meeting) only where the organization set `allow_unattended_send`; the sweep sends promoted shapes alone and nothing else; "Send" on a card, a "Ready to send" section, "Sent, confirmed" in a "Done" section; the send query reads the thread's current pending item, since the sweep rewrites items and orphans the draft's own link; the demo world is shared by the API and the worker through one state file, or the person's Send would look for a draft the worker's sweep made; 7 unit, 7 integration, 7 drilled |
...
**Corrections: the agent learns from what the person changed (2026-09-24).**
The owner asked whether learning from edits was for email only. It would
have been, because email is the only text the agent writes today, and that
was the wrong shape to build on. What was built instead is general.

Everywhere the agent proposes something, the person answers with what they
actually did. That pair is the same shape whatever the output is, so there
is one record for it (`corrections`, migration 0020): the kind of output, a
reference to the thing, the person it concerned, a short list of signals in
plain tokens, and a summary of numbers and forms. Never the text; the draft
and the message stay where they are, encrypted. Three producers exist
today, and any capability added later produces the same record:

- A draft against the message the person sent. The comparator
  (`voice-engine/src/corrections.ts`) reads what kind of change it was, not
  how much: shorter or longer, the greeting and sign-off used instead (as
  forms, "Hi {name}," and "Best, Alex", never the recipient), the
  pleasantries at the top dropped. Recorded when a sent message is matched
  to its draft, where the edit ratio was already being measured.
- A proposed Salesforce field against a decline ("Not now" on the
  proposal) or a hand edit (the field changed in Salesforce before the
  write, which already made the action stale).
- A routine's expected steps against the steps the person took, from the
  shadow comparison: skipped and added, by capability.

One inference over all of them, beside the inference from decisions and
with the same lifecycle: the same change three times becomes a proposal in
"Your agent thinks", in the person's words, with the evidence. "Sign off
with 'Best, Alex'." "Keep drafts short, about 60 words," from the median of
what they sent. "Open with 'Hey first name'." "Skip the pleasantries at the
top." "Don't propose changes to the close date," counting declines and
hand edits together. Two new rule kinds carry them: `style`, which is not
enforced by detection but whose sentence goes to the writer as the person's
own instruction once kept, and which keeps the same thing from being
proposed twice; and `skip_field`, enforced where proposals are made, so a
kept one leaves that field out before anything is compared. And the message
the person sent after editing a draft now leads the voice examples for that
contact and in general, ahead of older mail, because it is the best example
there is of what they wanted.

Tests. Comparator (3): forms without the person, each change as a signal,
sent as written is no signal. Inference (5): three of the same sign-off and
not two, the length from the median, the opener and greeting, declines and
hand edits counted together, never proposing an existing rule; the
skip-field rule found in scope. Repository (2): one correction per thing,
listed by kind, a colleague reads none; the message sent after an edit
found and one sent as written not. Sweep (3, a fresh person): three sent
drafts leave three corrections with the right signals and none of the
text, and become three proposals, proposed once; kept, the sign-off reaches
the writer's instructions and the sent version leads the voice; three
declined close dates become "don't propose the close date" and, kept, the
next proposal carries the next step only. Three drilled: the sign-off no
longer read, one change made enough, the kept rule not honoured. Each fails
the test written for it.

Not exercised: the model writer honouring a style instruction on real mail;
the deterministic writer takes preferences the same way the model does, and
the instruction reaches both.

Gate at this point: lint 26/26 · typecheck 26/26 · unit 25/25 · integration
**256** (sync 72, worker 11, db 100, api 73) · build 5/5.

**Phase 3, step 5 begins: sending mail, behind a per-person promotion
(2026-09-25).** The first capability added because a found routine needs
it: "they reply, you reply" can only run alone once the agent may send.
Sending is unlike every write before it, because it cannot be taken back,
so it gets a stronger gate than any of them.

The rule, stated plainly. The agent never sends by default; its output stays
a draft in Gmail. A send is a proposal like a CRM write: the exact message,
approved by the person, applied once, read back, recorded. A person may
promote one shape of send to run without asking, where a shape is the
situation the draft answers (a reply they owe, a thread gone quiet, a note
after a meeting). That promotion is theirs, written to their intent store
where they can read and withdraw it, and it is allowed only where the
organization has set `allow_unattended_send`, which is off by default. Four
conditions, none implying another.

What was built:

- `gmail.send_draft` in the catalog: high risk, not idempotent, never
  retried, not reversible. `gmail.send` in the action policy, unattended
  only under `allow_unattended_send`; the medium list does not open it.
- The adapter (`connector-adapters/src/gmail-send.ts`): sends by draft id
  through Gmail's drafts.send, and reads the message back by id. The
  demo world sends a draft as a message on its thread and answers the
  read-back.
- The action (`sync/src/actions.ts`): the diff is the draft as it is, with
  the text as a hash and its length, the recipient, the subject, and the
  situation. The text itself stays in the drafts table, encrypted; the
  page shows it in full to the person about to approve it. At apply time
  three things are checked again: the draft is still unsent, its text still
  hashes to what was approved, the thread has not moved since it was
  written. Then one send, and a read-back that the message exists on the
  thread and is marked sent. Verified, the draft becomes a sent message as
  written, and the receipt says there is no undo. A read-back that disagrees
  is a failure, and the draft is still treated as sent so it can never go
  out twice. A send that fails before Gmail takes it has an unknown result,
  and is never proposed again, by the person or the sweep.
- The sweep (`autoSends`): every unsent draft whose item is still pending is
  looked at; only a promoted shape under an open organization gate goes,
  and it goes through the same propose, approve, apply and read-back. A
  reply-owed draft is a different shape from a gone-quiet one.
- The page: "Send" on a card beside "Open draft in Gmail"; a "Ready to
  send" section showing to, subject and the full text, with "Send now",
  "Always" only where the organization allows it, and "Not now"; "Sent,
  confirmed" among the outcomes with "No undo for a sent email". The
  Connections copy now says it sends only what the person approves or told
  it to always send.

Two things the running demo showed that the tests had not. First, the
draft's own link to its item is set to null every sweep, because the sweep
rewrites pending items, so the send query found no situation and refused
every draft older than one pass. It now reads the thread's current pending
item; the situation a send answers is what is pending now, not the row the
draft was written against. Second, the API and the worker each held their
own scripted world, so the draft the worker's sweep made did not exist
where the person pressed Send, and the send failed as "provider". The demo
world now keeps what the product wrote (drafts, sent messages, tasks,
opportunity fields) in one file both processes read before every request
and write after every write (`DEMO_WORLD_STATE_FILE`, default in the OS
temp dir); the story itself is never stored. Sent message ids are named
after the draft so every process agrees on them. And the receipts moved
out from under the "Salesforce" heading into a "Done" section of their own,
since a sent email is not a Salesforce change; a failed send reads "Not
sent", not "Not written". The catalog's old guard, "never contains send",
was rewritten to what is still true: no delete, payment or transfer, and
the one send high risk, never retried, not reversible. The comments that
said no send exists were corrected in the same change.

Tests. Policy (2): the send kind allowed but not unattended by default, open
only through the gate, not through the medium list, disabled when listed.
Catalog (1): the only send is the gated draft send with the risk, retry
and reversibility above. Adapter (4): send by draft id once and read back;
a gone draft permanent, a 401 refreshed once; the demo world; two worlds on
one store, a draft made in one sent from the other and read back in both,
a third started later refusing to send it again. Sweep (6): the exact text
shown, one send, read back, verified, the audit chain valid, never twice;
an edited draft stale, a moved thread refused, a read-back that disagrees a
failure and the draft still treated as sent; "Always" refused while the
gate is shut, then two gone-quiet drafts go alone and the reply-owed one
waits; a send that fails before Gmail takes it is never proposed or swept
again; a colleague cannot send it; the sweep rewriting the item does not
lose the draft. HTTP (1). Seven drilled: the text hash not checked, a moved
thread sent over, the read-back ignored, a failed send proposed again, the
gate open by default, the old join on the draft's own link, the store not
read before a request. Each fails the test written for it. The fourth
needed a test of its own first: the unconfirmed-send path already marks
the draft sent, so the no-retry rule only shows when the send fails before
Gmail takes it.

Walked on the running demo: Send on a card, "Always" refused with the gate
shut, approve bound to the hash, "Sent, confirmed" with the message id,
Send again refused, the sent message on the thread at the next sync and
the item cleared.

Not exercised: a real Gmail send. Not built: sending mail a routine drafted
in supervised mode without the person's click; that is this promotion
applied to a routine's draft, which the sweep already does for any promoted
shape.

Gate at this point: lint 26/26 · typecheck 26/26 · unit 47/47 · integration
**263** (sync 78, worker 11, db 100, api 74) · build 5/5.
...
5. _Capabilities as routines need them._ Send ✅ (behind a per-person
   promotion the organization must allow, and per-message approval until
   promoted; see the note). Stage move, Slack, HubSpot to come. Each is an
   adapter through the catalog with its risk and reversibility, never a
   workflow.
