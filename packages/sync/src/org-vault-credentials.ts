...
/**
 * Vault-backed credentials for the ORGANIZATION's connectors (Salesforce,
 * HubSpot, Sheets): loaded from the envelope-encrypted `connector_accounts`
 * vault under `withTenant`, decrypted in-process, handed to the adapter as a
 * Bearer header, and NEVER returned to any caller, written into an
 * AgentSpec/output, or logged. Refresh uses the connector-auth refresh path
 * and re-persists the re-encrypted token.
 *
 * Lives here, not in the worker, because the API needs it too (the on-demand
 * sync asks the CRM), and apps may not import apps. The per-person mailbox
 * counterpart is user-vault-credentials.ts.
 */

export type OrgVaultDeps = {
  sql: Sql;
  masterKey: Buffer;
...
export function createOrgVaultCredentialProvider(deps: OrgVaultDeps): CredentialProvider {
